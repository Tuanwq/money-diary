...
export function DailyFinancialSummary({ summary }: { summary: Summary }) {
  return <section className="photo-finance-daily-summary" aria-label="Tổng kết tài chính trong ngày">
    ...
    <div><span>Chi</span><strong className="photo-finance-outflow">−{formatMoney(summary.expense)}</strong></div>
    <div><span>Thực nhận</span><strong className={summary.net < 0 ? "photo-finance-outflow" : undefined}>{summary.net > 0 ? "+" : summary.net < 0 ? "−" : ""}{formatMoney(Math.abs(summary.net))}</strong></div>
  </section>;
}
